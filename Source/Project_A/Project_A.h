...
#define LOG_NETMODEINFO \
	((GetNetMode()) == ENetMode::NM_Client) ? *FString::Printf(TEXT("CLIENT%d"), GPlayInEditorID) : ((GetNetMode() == ENetMode::NM_Standalone) ? TEXT("STANDALONE") : TEXT("SERVER"))
...
DECLARE_LOG_CATEGORY_EXTERN(LogPANetwork, Log, All);
DECLARE_LOG_CATEGORY_EXTERN(LogPATeleport, Log, All);
